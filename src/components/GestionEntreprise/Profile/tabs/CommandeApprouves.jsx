...
import { getCommandeApprouveClientsEntreprise, getCommandeECDVClientsEntreprise, getCommandeRejeteClientsEntreprise, setValidationStateCommande } from "../../../../services/entrepriseFunctionService";
...
export default function CommandeApprouves({ className }) {
...
<<<<<<< HEAD
  ...
  useEffect(() => {
    const fetchOrders = async (id) => {
      setLoading(true);
      try {
        let res = await getCommandeApprouveClientsEntreprise(id);
        setOrders(res);
      } catch (error) {
...
=======
  ...
  const fetchCommandeRejetceted = async (id) => {
    setLoading(true)
    try {
      const res = await getCommandeApprouveClientsEntreprise(id);
      console.log("res")
      console.log(res)
      setOrders(res);
    } catch (error) {
    ...
    <>
      <div className="bg-white shadow-md rounded-lg p-6">
        <div className="flex items-center mb-6">
          <h2 className="text-2xl font-bold">Commandes validées</h2>
        </div>
