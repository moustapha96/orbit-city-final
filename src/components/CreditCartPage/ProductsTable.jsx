...
import InputQuantityCreditCommande from "../Helpers/InputQuantityCreditCommande";

export default function ProductsTable({ className }) {

  const { creditOrder, clearCreditOrder, removeFromCreditOrder } =
      ...
      <div className="relative w-full overflow-x-auto border border-[#EDEDED]">
        <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
          <tbody>
            ...
            {/* table heading end */}
            {creditOrder && creditOrder.map((produit, index) => (
              <tr key={index} className="bg-white border-b hover:bg-gray-50">
                ...
                <td className="text-center py-4 px-2">
                  <div className="flex space-x-1 items-center justify-center">
                    <span className="text-[15px] font-normal">
                      {" "}
                      {formatPrice(produit.creditorder_price)}
                    </span>
                  </div>
                </td>
                ...
                <td className=" py-4">
                  <div className="flex justify-center items-center">
                    <InputQuantityCreditCommande produit={produit} />
                  </div>
                </td>
                ...
                <td className="text-right py-4">
                  <div className="flex space-x-1 items-center justify-center">
                    <span className="text-[15px] font-normal">
                      {" "}
                      {formatPrice(produit.creditorder_price * produit.quantity)}
                    </span>
                  </div>
                </td>
